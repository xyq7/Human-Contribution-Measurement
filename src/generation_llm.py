...
def process_dataset(file_name, num_examples):
    ...
    Returns:
        ...
        subject_dataset: Dataset containing unique subjects (if applicable)
    """
        ...
        if output_file.exists() and args.resume:
            needed_messages = Counter(
                [
                    f"{i}_{mode}"
                    for i, mode in zip(
                        processed_dataset["id"], processed_dataset["mode"]
                    )
                ]
            )
            ...
            def filter_fn(example):
                return f"{example['id']}_{example['mode']}" not in exist_messages
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            ...
            while retry_time < MAX_RETRY:
                ...
                for id_name, response, message, mode in zip(
                    dataset["id"],
                    responses,
                    dataset["message"],
                    dataset["mode"],
